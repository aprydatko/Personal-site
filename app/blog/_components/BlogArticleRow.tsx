...
export const BlogArticleRow = ({ article }: { article: BlogPost }) => (
  <article className="grid gap-5 border-b border-border-subtle py-[clamp(2rem,4vw,3rem)] first:pt-4 sm:grid-cols-[10rem_1fr_auto] sm:items-start sm:gap-[clamp(2rem,5vw,3rem)] sm:py-[clamp(2rem,4vw,3rem)] lg:grid-cols-[minmax(10rem,0.62fr)_minmax(19rem,1.25fr)_7rem]">
    <ArticlePreview />
    <div className="py-[clamp(0.5rem,3vw,1rem)]">
      ...
      <p className="mt-4.5 max-w-md text-md font-medium leading-relaxed text-muted">
        {article.description}
      </p>
    </div>
    <div className="mt-18 flex items-center justify-end gap-16 whitespace-nowrap font-mono text-sm text-muted">
      <span>{article.readingTime}</span>
      <MoveRight className="shrink-0 !h-5 !w-5 stroke-[1.75] text-foreground" aria-hidden="true" />
    </div>
  </article>
);
